...
const finalMessages = [
  ...
  "Passou pelos corações, pelas estrelas, pelo quiz, e por algumas brincadeiras bobas e ainda ficou tentando uma senha que, sinceramente, nunca existiu.",
...
// Sem isso, girar o celular (ou redimensionar a janela) deixava o fundo
// de estrelas borrado/esticado e o canvas do jogo com o tamanho errado.
window.addEventListener("resize", () => {
  drawStars();
  if (canvas && document.getElementById("game").classList.contains("active")) {
    setupCanvas();
  }
});
...
function endPhase() {
  ...
  setTimeout(() => {
    ...
    if (phase === 1) {
      ...
      showMessage(
        "Impressionante.",
        "Tá indo muito bem amor. Agora tem mais um joguinho aqui que vai te estressar também 😈",
        "Continuar",
        startMemory
      );
    }
...
const quizBank = [
  ...
  },
  {
    ...
    answer: 0,
    correctMsg: "isso, Flamengo, sempre Flamengo",
  ...
  },
  {
    ...
    answer: 1,
    correctMsg: "isso, estrogonofe, você sabe muito bem",
  ...
  },
  {
    ...
    answer: 1,
    correctMsg: "Stranger Things, isso mesmo amor",
        ...
        if (memMatched === MEMORY_EMOJIS.length) {
          ...
          setTimeout(() => {
            showMessage(
              ...
              `terminou com ${memoryErrors} erro(s). Tá bom amor, nao vou subestimar voce mais. Agora vamos ver se você sabe sobre mim.`,
              "Começar quiz",
              startQuiz
            );
          }, 600);
...
function startPatienceTest() {
  ...
  simBtn.onclick = () => {
    ...
    showMessage(
      "kkkkkk 😏",
      "KKKKKKKKKKK sinto muito bb, aqui nesse joguinho quem manda sou eu 🤣",
      "Continuar",
      () => showMessage(
        "🎁 quase lá",
        "Por você ter chegado até aqui, você merece um prêmio.",
        "Continuar",
        startFakeLoading
      )
    );
...
// a "fugir" do toque/clique a partir da primeira tentativa de toque.
// Bloqueia TODOS os eventos do gesto (pointerdown/up, click, touch) pra
// impedir que um clique sintético do navegador escape pro botão Sim
// que fica embaixo, ou ative qualquer ação indevida.
// Guarda o lugar de origem (lado a lado com o "Sim") de cada botão fujão,
// pra poder devolver ele lá antes de clonar de novo numa próxima partida —
// senão, depois da 1ª fuga, ele ficaria "preso" dentro do <body> pra sempre.
const dodgeHomes = {};

function setupDodgingButton({ btnId, onDodge }) {
  let oldBtn = document.getElementById(btnId);

  if (!dodgeHomes[btnId]) {
    dodgeHomes[btnId] = { parent: oldBtn.parentNode, next: oldBtn.nextSibling };
  } else {
    const home = dodgeHomes[btnId];
    if (oldBtn.parentNode !== home.parent) {
      home.parent.insertBefore(oldBtn, home.next);
    }
  }
  ...
  function activateDodging() {
    ...
    // Mede a posição e o tamanho ANTES de mudar pra position:fixed,
    // senão o rect já vem errado (o layout muda ao sair do flex row).
    const rect = btn.getBoundingClientRect();

    btn.style.width = rect.width + "px";
    btn.style.height = rect.height + "px";

    // CORREÇÃO DO BUG: o .card usa backdrop-filter, e isso cria um novo
    // "containing block" pra qualquer descendente position:fixed (regra do
    // CSS, vale igual pra filter/backdrop-filter/transform/will-change).
    // Ou seja: o botão NÃO estava fugindo pela tela inteira, e sim ficando
    // preso e mal posicionado dentro dos limites do card — por isso ele
    // "desaparecia" ou só dava pra clicar numa pontinha/canto específico.
    // Solução: mover o botão pra ser filho direto do <body> antes de
    // ativar o position:fixed, assim ele escapa desse containing block
    // errado e passa a fugir pela tela inteira de verdade, pra sempre.
  ...
  function dodge(e) {
    ...
    // Evita disparo duplicado quando dois eventos do mesmo gesto
    // (ex.: pointerdown + click sintético) chegam quase juntos.
    const now = Date.now();
    if (now - lastDodgeTs < 80) return;
    lastDodgeTs = now;
  ...
  // Escuta todos os tipos de gesto possíveis e bloqueia cada um deles,
  // garantindo que nenhum clique sintético escape pro elemento de baixo.
...
function startFakeLoading() {
  ...
  document.querySelector("#loading h2").textContent = "Processando seu prêmio, aguarde um pouco chata";
